...
<%@page import="insuranceModel.PolicyHolder"%>
...
<body>
	...
	<div class="table-container">
		<table class="table">
			...
			<tbody>
				<%
				List<PolicyHolder> PolicyHolder =(List<PolicyHolder>)request.getAttribute("policyHolder");
                 for(PolicyHolder ph:PolicyHolder){
			
				%>
				<tr>
					<th scope="row"><%=ph.getId() %></th>
					<td><%=ph.getName() %></td>
					<td><%=ph.getDob() %></td>
					<td><%=ph.getContact() %></td>
					<td><%=ph.getAddress() %></td>
					<td><%=ph.getEmail() %></td>
					<td><%=ph.getPassword() %></td>
				</tr>
				<%} %>
			</tbody>
		</table>
